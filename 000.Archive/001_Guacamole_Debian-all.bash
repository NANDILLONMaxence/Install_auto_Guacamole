...
guacamole_server_link="https://archive.apache.org/dist/guacamole/1.5.0/source/guacamole-server-1.5.0.tar.gz"
guacamole_webapp_link="https://archive.apache.org/dist/guacamole/1.5.0/binary/guacamole-1.5.0.war"
guacamole_auth_jdbc_link="https://archive.apache.org/dist/guacamole/1.5.0/binary/guacamole-auth-jdbc-1.5.0.tar.gz"
guacamole_auth_jdbc="guacamole-auth-jdbc-mysql-1.5.0.jar"
mysql_connector_link="https://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-j-8.3.0.tar.gz"
...
# à revoir ! pas opti.
check_distribution() {
    show_message "Checking distribution compatibility..."
    distribution=$(lsb_release -d | cut -f2)
    if ["$distribution" = "Debian GNU/Linux 10 (Buster)"] || [ "$distribution" = "Debian GNU/Linux 11 (bullseye)" ] || [ "$distribution" = "Debian GNU/Linux 12 (Bookworm)"] ; then
        show_message "The distribution is compatible."
    else
        read -p "The distribution is not Debian. Do you want to continue despite the version incompatibility, if you have an Ubuntu distribution install this lib libjpeg-turbo8-dev delete libjpeg62-turbo-dev in the srcipt line 45 and restart this script ? (yes/no): " choix_utilisateur

        if [ "$choix_utilisateur" = "yes" ] || [ "$choix_utilisateur" = "y" ]; then
            show_message "Continuation of the script despite version incompatibility."
        else
            show_message "Stopping the script."
            exit 1
        fi
    fi
}
...
install_prerequisites() {
    ...
    if [ $? -ne 0 ]; then
        show_message "Failed to install prerequisites. Exiting."
        exit 1
    fi
...
install_guacamole_server() {
    ...
    if [ $? -ne 0 ]; then
        show_message "Failed to install Guacamole Server. Exiting."
        exit 1
    fi
...
install_tomcat9() {
    ...
    if [ $? -ne 0 ]; then
        show_message "Failed to install Tomcat9. Exiting."
        exit 1
    fi
...
deploy_guacamole_webapp() {
    ...
    if [ $? -ne 0 ]; then
        show_message "Failed to deploy Guacamole Web App. Exiting."
        exit 1
    fi
...
install_mariadb() {
    ...
    if [ $? -ne 0 ]; then
        show_message "Failed to install MariaDB Server. Exiting."
        exit 1
    fi
...
main() {
    ...
    show_message "Guacamole installation completed successfully!"
    echo "---------------------------------------------------------------"
    echo ""
